...
This document explains how the assistant chat works end-to-end in this branch.
...
1. Chat pipeline:
- User prompt to uiChatResource to backend api chat to LLM stream to UI message rendering.

2. Domain action pipeline:
- LLM tool decision to frontend tool handler to store service router updates to tool result to LLM follow-up response to widget rendering.
